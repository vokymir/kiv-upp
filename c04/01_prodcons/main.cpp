...
public:
  ...
  inline size_t get() { return mCount; }
};
...
private:
  ...
  // TODO: doplnit
  CSemaphore s_full{0};
  CSemaphore s_empty{Max_Work_Items};
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < Producers_Count; i++)
    threads.push_back(
        std::make_unique<std::thread>(&Producer, std::ref(queue)));

  for (size_t i = 0; i < Consumers_Count; i++)
    threads.push_back(
        std::make_unique<std::thread>(&Consumer, std::ref(queue)));
