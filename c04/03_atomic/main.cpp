...
// (1) nezamykano, nechraneno - vysledek bude spatny, ale nejrychleji (to je zradne!)
int sum;
// (2) nezamykano, nechraneno, volatilni pamet - vysledek bude spatny, o neco pomalejsi, ale stale docela rychly
//volatile int sum;
// (3) zamykano - vysledek bude spravny, ale vypocet bude trvat dele
//LockableInt sum;
// (4) nezamykano, ale zajistena atomicita (CAS, LL/SC, ...) - spravny vysledek; proc je ale casto pomalejsi nez (3)?
//std::atomic<int> sum;
...
int main(int argc, char** argv)
{
	...
	for (size_t i = 0; i < Worker_Count; i++)
		workers.push_back(std::make_unique<std::thread>(&Summator, i*step, (i+1)*step));
	...
	std::cout << "Celkem:    " << std::chrono::duration_cast<std::chrono::milliseconds>(tp_end - tp_start).count() << "ms" << std::endl;
