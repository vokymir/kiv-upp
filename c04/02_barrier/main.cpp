...
class CBarrier
{
	private:
		...
		// POZOR: tady je klicove slovo volatile naduzito! V soucasne implementaci (aktivni cekani) je ale potreba, jinak kompilator cyklus vyoptimalizuje
...
int main(int argc, char** argv)
{
	...
	for (size_t i = 0; i < Thread_Count; i++)
		threads.push_back(std::make_unique<std::thread>(&Thread_Func, std::ref(barrier)));
