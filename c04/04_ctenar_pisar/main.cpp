...
/*
 * Vlaknova funkce ctenare - cte z databaze, overuje, ze soucet hodnot je stale 120
 */
void ctenar(size_t idx, Database& db) {
	...
	for (size_t i = 0; i < Pocet_Operaci; i++) {
		...
		Casy_Ctenari[idx] += std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
...
/*
 * Vlaknova funkce pisare - zapisuje do databaze, zvetsuje a hned zmensuje kazdou hodnotu o 1
 */
void pisar(size_t idx, Database& db) {
	...
	for (size_t i = 0; i < Pocet_Operaci; i++) {
		...
		auto end = std::chrono::high_resolution_clock::now();
		Casy_Pisari[idx] += std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();

		// = tohle znamena, ze po zasahu pisare se obsah DB nezmeni (v idealnim pripade)
	}
}
...
int main(int argc, char** argv) {
	...
	std::cout << "Prumerny cas ctenaru: " << prum_cas_ctenaru << " us" << std::endl;
