...
// vlaknova funkce generatoru - generuje cisla a notifikuje druhe vlakno k
// jejich zpracovani nasledne zacne pocitat dalsi vysledek a nez ho preda, tak
// pocka, az si druhe vlakno vysledek vyzvedne a notifikuje toto vlakno, ze uz
// muze predat dalsi vysledek
...
// druhe vlakno - ceka na vysledek od prvniho vlakna, 'nejak' ho zpracuje a
// vyprazdni je ukonceno az kdyz prvni vlakno nastavi priznak konce
void *thread_2(void *par) {
  ...
  do {
    ...
    if (calc_result == RESULT_FLAG_EXIT) {
      ...
      // tady to sice nema takovy smysl - uz nikdo zamykat nebude, ale zvykneme
      // si na to
      pthread_mutex_unlock(&calc_mtx);
      break;
    }
    ...
    // vyzvedneme si vysledek a sdilenou promennou nastavime na prazdno (aby ji
    // mohlo prvni vlakno zase modifikovat)
