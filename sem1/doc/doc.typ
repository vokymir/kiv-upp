...
V této kapitole je primárně rozebírán problém paralelizace, tedy ne jak
...
optimalizaci připisuji mezivláknovým přístupům do stejné oblasti paměti. Všechna
vlákna prochází všechny stanice, takže si nejspíš vzájemně zneplatňují cache.

Po analýze precedenčního grafu jsem zavedl paralelismus pomocí vlákeného
rybníčku i do funkce `parallel_version` v souboru `main.cpp`. Empirickým
testováním (průměr ze 100 běhů) jsem ale zjistil, že verze *s rybníčkem* je
pomalejší o vyšší desítky milisekund.
