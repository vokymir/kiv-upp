...
bool CRouter::Optimize_Node_Order(const std::list<size_t>& nodeIds, size_t inputNode)
{
	...
	auto get_distance = [this](size_t nsrc, size_t ndst) {
		return std::sqrt(std::pow(m_nodes[nsrc].x - m_nodes[ndst].x, 2.0) + std::pow(m_nodes[nsrc].y - m_nodes[ndst].y, 2.0));
	};
	...
	auto find_path = [this, &clear_temps](size_t from, size_t to) {
		...
		while (!discovered.empty())
		{
			...
			for (size_t s : discovered)
			{
				...
				{
					if (m_nodes[e.dest_id].discovered_from == InvalidNode && m_nodes[s].path_metric + e.cost < nextcost)
					{
						selected = s;
						next = e.dest_id;
...
bool CRouter::Draw_Path()
{
	...
	svg_out << "<svg width=\"" << Grid_Size_X * Grid_Spacing_X * Scaling << "\" height=\"" << Grid_Size_Y * Grid_Spacing_Y * Scaling << "\" xmlns=\"http://www.w3.org/2000/svg\">" << std::endl;
	{
		svg_out << "<g>" << std::endl;
		for (const auto& n : m_nodes)
		{
			const std::string& col = is_warehouse(n.second.id) ? "red" : (is_internode(n.second.id) ? "blue" : "black");

			svg_out << "<circle cx=\"" << static_cast<int>(n.second.x * Scaling) << "\" cy=\"" << static_cast<int>(n.second.y * Scaling) << "\" r=\"" << (is_warehouse(n.second.id) ? 3 : 2) << "\" fill=\"" << col << "\"><title>" << n.second.id << " - " << n.second.name << "</title></circle>" << std::endl;
			...
			for (const auto& e : n.second.edges)
			{
				...
				svg_out << "<line x1=\"" << static_cast<int>(n1.x * Scaling) << "\" y1=\"" << static_cast<int>(n1.y * Scaling) << "\" x2=\"" << static_cast<int>(n2.x * Scaling) << "\" y2=\"" << static_cast<int>(n2.y * Scaling) << "\" stroke-width=\"0.5\" stroke=\"" << col << "\" />" << std::endl;
			}
		}
		svg_out << "</g>" << std::endl;
	}
