...
bool CServer::Init(const std::string &basePath,
                   const std::string &listenAddress, const int listenPort) {
  ...
  m_server->Get("/", std::bind(&CServer::Handle_Get_Any, this,
                               std::placeholders::_1, std::placeholders::_2));
  m_server->Post("/submit", static_cast<httplib::Server::Handler>(std::bind(
                                &CServer::Handle_Post_Form, this,
                                std::placeholders::_1, std::placeholders::_2)));

  m_server->set_error_handler(
      [](const httplib::Request &req, httplib::Response &res) {
        // pokud je status kod 500, uz jsme osetrili chybu jako vyjimku v
        // handleru nize
        if (res.status != 500) {
  ...
  m_server->set_exception_handler([](const httplib::Request &req,
    ...
    try {
      ...
      res.set_content("Doslo k vyjimce pri zpracovani pozadavku: " +
                          std::string(ex.what()),
                      "text/plain");
    } catch (...) {
...
bool CServer::Run() {
  ...
  bool success =
      m_server && m_server->bind_to_port(m_listenAddress.c_str(), m_listenPort);

  if (success) {
    std::cout << "Server posloucha na http://" << m_listenAddress << ":"
              << m_listenPort << "/" << std::endl;
    std::cout << "Tuto stranku zobrazte v prohlizeci a zadejte URL adresy, "
                 "ktere chcete zpracovat."
              << std::endl;
  } else {
...
void CServer::Handle_Get_Any(const httplib::Request &req,
                             httplib::Response &res) {
  // jen odesle nacteny obsah souboru index.html
  res.set_content(m_servedPage, "text/html");
}

void CServer::Handle_Post_Form(const httplib::Request &req,
                               httplib::Response &res) {
