...
private:
  ...
  std::function<void(const std::vector<std::string> &, std::string &)>
      m_onURLsReceived;
...
public:
  ...
  bool Init(const std::string &basePath,
            const std::string &listenAddress = DefaultAddress,
            const int listenPort = DefaultPort);

  // registrace callbacku pro zpracovani odeslanych URL
  // onURLsReceived - callback pro zpracovani odeslanych URL
  void RegisterFormCallback(
      const std::function<void(const std::vector<std::string> &, std::string &)>
          &onURLsReceived) {
